...
from somo_rl.utils.customized_callbacks import Obs_Img_NN, Obs_Img_RNN
...
class Policy_rollout:
    ...
    def train(self, train_loader):
        ...
        for data, target in train_loader:
            ...
            observation = self.obs_img_model(data.view(self.run_config["max_episode_steps"], -1, data.shape[-1]).to(th.float32))

            loss = self.criterion(observation, target.view(self.run_config["max_episode_steps"], -1, target.shape[-1]).to(th.float32))
            loss.backward()
            self.optimizer.step()
    ...
    def load_rollouts(self):
        ...
        if os.path.isfile(saved_data_path):
            ...
            list_observations = []
            list_actions = []
            if os.path.isfile(os.path.join(models_dir, "best_obs_model_mlp")):
                ...
                for _ in tqdm(range(1000)):
                    ...
                    for _ in range(self.run_config["max_episode_steps"]):
                        ...
                        obs, hidden = obs_img_mlp_model(input_data.view(1, 1, input_data.shape[0]), hidden) # (seq, batch, hidden)
                ...
                if os.path.isfile(os.path.join(self.run_dir, f"obs_model_input_0")):
                    self.dataset = th.utils.data.TensorDataset(th.load(os.path.join(self.run_dir, f"obs_model_input_0")), th.load(os.path.join(self.run_dir, f"obs_model_target_0")))
                    train_loader = th.utils.data.DataLoader(dataset=self.dataset, batch_size=5, shuffle=False)
                    for epoch in range(1, self.epochs + 1):
                        loss = self.train(train_loader)
                        print(f"@@@@@@ obs_img_model Training epoch: {epoch}, Loss: {loss.item():.6f}")
                        self.scheduler.step()
                        mean_reward_obs, std_reward_obs, mean_z_rotation_obs, std_z_rotation_obs = self.evaluate()
                        print(f"@@@@@@ Eval obs_img_model epoch: {epoch}, episode_reward={mean_reward_obs:.2f} +/- {std_reward_obs:.2f}, episode_z_rotation={mean_z_rotation_obs:.2f} +/- {std_z_rotation_obs:.2f}")
                ...
                for epi in tqdm(range(1000)):
                    ...
                    for _ in range(self.run_config["max_episode_steps"]):
                        ...
                        input_data = th.cat((th.from_numpy(obs).to(self.device), th.from_numpy(action).to(self.device)), -1)
                        obs, _, _, info = self.env.step(action)
                        target_data = th.from_numpy(obs).to(self.device)
                        list_actions.append(th.tensor(action))
                        episode_input.append(input_data.view(1, input_data.shape[0]))
                        episode_target.append(target_data.view(1, target_data.shape[0]))
                    # self.dataset = th.utils.data.TensorDataset(th.stack(episode_input), th.stack(episode_target))
                    # self.dataset.append(th.utils.data.TensorDataset(th.stack(episode_input), th.stack(episode_target)))
                    input.append(th.stack(episode_input, 1))
                    target.append(th.stack(episode_target, 1))
                    print("@@@@@@", th.cat(input).shape, th.cat(target).shape)
                self.dataset = th.utils.data.TensorDataset(th.cat(input), th.cat(target))
                th.save(th.cat(input), os.path.join(self.run_dir, "obs_model_input"))
                th.save(th.cat(target), os.path.join(self.run_dir, "obs_model_target"))
                train_loader = th.utils.data.DataLoader(dataset=self.dataset, batch_size=5, shuffle=False)
                for epoch in range(1, self.epochs + 1):
                    loss = self.train(train_loader)
                    print(f"@@@@@@ obs_img_model Training epoch: {epoch}, Loss: {loss.item():.6f}")
                    self.scheduler.step()
                    # mean_reward_obs, std_reward_obs, mean_z_rotation_obs, std_z_rotation_obs = self.evaluate()
                    # print(f"@@@@@@ Eval obs_img_model episode: {epi}, episode_reward={mean_reward_obs:.2f} +/- {std_reward_obs:.2f}, episode_z_rotation={mean_z_rotation_obs:.2f} +/- {std_z_rotation_obs:.2f}")
                # train_loader = th.utils.data.DataLoader(dataset=th.utils.data.ConcatDataset(self.dataset), batch_size=100, shuffle=True)
                self.env.close()
                ...
                th.save(self.obs_img_model, os.path.join(models_dir, "best_obs_model_mlp"))
                mean_reward_obs, std_reward_obs, mean_z_rotation_obs, std_z_rotation_obs = self.evaluate()
                print(f"@@@@@@ Eval obs_img_model episode: {epi}, episode_reward={mean_reward_obs:.2f} +/- {std_reward_obs:.2f}, episode_z_rotation={mean_z_rotation_obs:.2f} +/- {std_z_rotation_obs:.2f}")
            # else:
            #     print(f"@@@@@@ Preparing data from the rl_agent and env from: {models_dir}")
